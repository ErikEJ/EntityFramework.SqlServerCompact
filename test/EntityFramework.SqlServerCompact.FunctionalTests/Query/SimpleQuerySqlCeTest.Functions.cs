...
FROM [Customers] AS [c]
WHERE [c].[ContactName] LIKE N'M' + N'%' AND (SUBSTRING([c].[ContactName], 1, LEN(N'M')) = N'M')");
        }
...
WHERE ([o].[CustomerID] = N'ALFKI') AND (CONVERT(bigint, CONVERT(bigint, [o].[OrderID] % 1)) >= CAST(0 AS bigint))",
                ...
                @"SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
FROM [Orders] AS [o]
WHERE ([o].[CustomerID] = N'ALFKI') AND (CONVERT(bigint, CONVERT(nvarchar(4000), [o].[OrderID] % 1)) >= CAST(0 AS bigint))");
        }
