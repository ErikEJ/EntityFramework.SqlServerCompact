...
namespace Microsoft.EntityFrameworkCore
{
    ...
    {
        protected SqlCeDatabaseModelFixture Fixture { get; }

        public SqlCeDatabaseModelFactoryTest(SqlCeDatabaseModelFixture fixture) => Fixture = fixture;
...
CREATE TABLE [MountainsColumns] (
    ...
    Primary Key (Name, Id)
        ...
        {
            var tables = Fixture.Query<string>("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Strings';");
            if (tables.Count() > 0)
            {
                Fixture.ExecuteNonQuery("DROP TABLE [Strings];");
            }
        ...
        {
            var tables = Fixture.Query<string>("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Identities';");
            if (tables.Count() > 0)
            {
                Fixture.ExecuteNonQuery("DROP TABLE [Identities];");
            }
        ...
        public DatabaseModel CreateModel(List<string> createSql, IEnumerable<string> tables = null)
            => Fixture.CreateModel(createSql, tables);
